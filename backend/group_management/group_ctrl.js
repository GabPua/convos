  ...
  return {
    ...
    inviteMembers: async (req, res) => {
      ...
      try {
        ...
        if (matchedCount) {
          await Group.updateOne({ _id: groupId }, { $addToSet: { invitations: { $each: filteredIds } } });

          for (let i = 0; i < filteredIds.length; i++) {
            if (users.has(filteredIds[i])) {
              io.to(users.get(filteredIds[i])).emit('invite');
            }
          }
      ...
      try {
        ...
        if (result.modifiedCount == 1) {
          await Group.findByIdAndUpdate(groupId, {
            ...
            $pull: { invitations: req.session._id }
          }).lean().exec();
          io.to(groupId).emit('update group');
          res.json({ result: true });
        } else {
      ...
      try {
        ...
        if (result && userId !== result.admin) {
          ...
          if (req.session._id === result.admin || req.session._id === userId) {
            ...
            io.to(users.get(userId)).emit('invite');
            io.to(req.params.id).emit('update group');
